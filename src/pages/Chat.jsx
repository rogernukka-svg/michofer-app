...
export default function Chat() {
  ...
  async function init() {
    ...
    if (!tripId || !currentUser) {
      setNotice('El chat se habilita cuando tenés un viaje activo.')
      return
    }
    ...
    const isTripParticipant =
      tripData.client_id === currentUser.id || tripData.driver_id === currentUser.id

    if (!isTripParticipant) {
      setNotice('No tenés acceso a este chat.')
      return
    }
  ...
  async function sendMessage(event) {
    ...
    const isTripParticipant =
      trip?.client_id === user.id || trip?.driver_id === user.id

    if (!isTripParticipant) {
      setNotice('No tenés permiso para enviar mensajes en este viaje.')
      return
    }
      ...
      <section className="phone chat-phone">
          ...
          <div className="chat-person">
            <div className="avatar-small">
              {otherProfile?.avatar_url ? (
                <img src={otherProfile.avatar_url} alt="Contacto" />
              ) : (
                <UserRound size={18} />
              )}
            </div>
        ...
        <section className="chat-trip-strip">
          ...
          <div>
            ...
            <span>
              {trip?.price
                ? `${Number(trip.price).toLocaleString('es-PY')} Gs.`
                : 'Mensajes del viaje'}
            </span>
          </div>
        </section>
        ...
        <section className="chat-messages" aria-label="Mensajes">
          {messages.length === 0 && (
            <div className="empty-state">
              Todavía no hay mensajes. Escribí para coordinar el punto.
            </div>
          )}
          ...
          {messages.map((message) => (
            <div
              key={message.id}
              className={message.sender_id === user?.id ? 'bubble me' : 'bubble'}
            >
              {message.body}
            </div>
          ))}
